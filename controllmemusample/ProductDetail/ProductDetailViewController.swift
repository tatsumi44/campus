...
class ProductDetailViewController: UIViewController,UITableViewDataSource, CoachMarksControllerDataSource, CoachMarksControllerDelegate ,UIScrollViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        statusheight = statusBarHeight + navBarHeight!
        print("statusheigh\(statusheight)")
        print("height\(UIScreen.main.bounds.size.height)")
        print("heigh - 60\(UIScreen.main.bounds.size.height - 60 - statusheight)")
        priceLabel.text = "¥\(productArray.price!)"
        priceLabel.frame = CGRect(x: 20, y: UIScreen.main.bounds.size.height - 60 - statusheight + 10, width: (UIScreen.main.bounds.size.width/3) - 10, height: 40)
        moveView.frame = CGRect(x: 0, y: UIScreen.main.bounds.size.height - 60 - statusheight, width: UIScreen.main.bounds.size.width, height: 60)
        
        purchasedButton.frame = CGRect(x: (UIScreen.main.bounds.size.width/3) + 10, y: UIScreen.main.bounds.size.height - 60 - statusheight + 10, width: (UIScreen.main.bounds.size.width/3)*2 - 20, height: 40)
        
        self.navigationItem.title = "商品の詳細"
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        moveView.frame = CGRect(x: 0, y: UIScreen.main.bounds.size.height - 60 - statusheight + scrollView.contentOffset.y, width: UIScreen.main.bounds.size.width, height: 60)
        purchasedButton.frame = CGRect(x: (UIScreen.main.bounds.size.width/3) + 10, y: UIScreen.main.bounds.size.height - 60 - statusheight + 10 + scrollView.contentOffset.y, width: (UIScreen.main.bounds.size.width/3)*2 - 20, height: 40)
        priceLabel.frame = CGRect(x: 20, y: UIScreen.main.bounds.size.height - 60 - statusheight + 10 + scrollView.contentOffset.y, width: (UIScreen.main.bounds.size.width/3) - 10, height: 40)
        print("スクロール量\(scrollView.contentOffset.y)")
    }
    //スクロール終了時
